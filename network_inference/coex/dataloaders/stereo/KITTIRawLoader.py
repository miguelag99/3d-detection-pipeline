...
class ImageLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            batch['imgL'], batch['imgR'] = left_img_p[:,:,:1000], right_img_p[:,:,:1000]
            batch['imgLRaw'], batch['imgRRaw'] = left_img_[:,:,:1000], right_img_[:,:,:1000]
            batch['x1'], batch['y1'] = x1, y1

            print('bacth imgL shape: ', batch['imgL'].shape)
